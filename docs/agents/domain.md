...
├── CONTEXT.md
├── docs/adr/
│   ├── 0001-packs-are-data-not-code.md
│   └── 0002-on-device-first-with-byo-key.md
└── <AppName>/          ← Swift sources
```

There is no `CONTEXT-MAP.md`. If this repo ever splits into genuinely separate contexts, that file appears at the root and points at one `CONTEXT.md` per context.
